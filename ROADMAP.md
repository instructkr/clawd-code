...
362. **`doctor --output-format json` auth check reports `warn` even when a valid CLI session token exists in `~/.claw/`; only probes environment variables** — dogfooded 2026-04-30 by Jobdori on `c6c01bea`. Running `./rust/target/debug/claw --output-format json doctor` on a machine where `ANTHROPIC_API_KEY` is not set but a valid `~/.claw/` session token exists returns the auth check as `warn` (not `ok`), indicating authentication is not configured. The doctor command probes only `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_API_KEY` environment variables and a legacy refresh-token path; it does not check `~/.claw/auth.json`, `~/.claw/sessions/`, or other CLI session token stores that the `--resume` flow successfully uses. This is a false positive: the machine can authenticate via `--resume latest /status` without any env var set, yet `doctor` reports auth as unhealthy. **Required fix shape:** (a) extend the auth check to probe `~/.claw/auth.json` or equivalent CLI session token file; (b) if a valid token is found there, report `status:"ok"` with `source:"cli_session_token"` rather than `warn`; (c) add `checked_paths[]` to the auth check result listing all locations probed; (d) add regression coverage confirming `doctor` returns `ok` for auth when a valid `~/.claw/` token is present and no env vars are set. Source: Jobdori live dogfood, mengmotaHost, `c6c01bea`, 2026-04-30.
