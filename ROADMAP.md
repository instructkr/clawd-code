...
346. **Top-level `agents list --output-format json` can hang with zero stdout/stderr, blocking native-agent inventory discovery without a typed timeout or partial status** — dogfooded 2026-04-29 for the 19:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `b2f7a335`. After rebuilding and verifying `./rust/target/debug/claw version --output-format json` reported `git_sha` `b2f7a335`, running `./rust/target/debug/claw agents list --output-format json` produced no stdout bytes and no stderr bytes, did not return within the probe window, and blocked subsequent command-surface probes until the outer process was killed. A direct bounded repro `timeout 20 ./rust/target/debug/claw agents list --output-format json` exited `124` with `stdout=0` and `stderr=0`. This is distinct from #328 and #329: those document wrong/flattened native-agent metadata after a response exists, while this pinpoint is a silent hang before any JSON inventory or error envelope is emitted. **Required fix shape:** (a) make `agents list --output-format json` return promptly with the same bounded inventory semantics as `agents --output-format json`, or explicitly reject the redundant `list` subcommand with a structured stdout JSON error; (b) add an internal timeout/guard around native-agent discovery so filesystem/root scanning cannot hang silently; (c) emit a typed `agent_discovery_started` / `agent_discovery_timeout` or final JSON error with root/source context when discovery stalls, without leaking secrets; (d) add regression coverage proving `agents list --output-format json` either completes with valid stdout JSON or returns a typed JSON error within a deterministic budget. **Why this matters:** native-agent inventory is a control-plane dependency for delegation. If the natural `agents list` spelling hangs with no bytes, claws cannot distinguish slow filesystem scan, provider/tool deadlock, malformed agent config, or CLI dispatch ambiguity, and agent selection automation loses a safe preflight path. Source: gaebal-gajae dogfood follow-up for the 19:30 nudge on rebuilt `./rust/target/debug/claw` `b2f7a335`; an earlier incorrect `mcp help` hang hypothesis was closed as invalid after immediate repro returned stdout JSON.
