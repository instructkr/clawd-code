...
390. **Top-level `skills list --help --output-format json` hangs with zero stdout/stderr even though `skills --help --output-format json` returns valid help JSON** — dogfooded 2026-04-30 for the 07:30 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. A fresh probe on the skills/plugin-adjacent discoverability surface first showed that `./rust/target/debug/claw skills --help --output-format json` exits `0` with parseable JSON (`kind:"skills"`, `action:"help"`, `usage`). But the natural subcommand-help form `timeout --kill-after=1s 8s ./rust/target/debug/claw skills list --help --output-format json` exits `124` with `stdout=0` and `stderr=0`. A follow-up loop also showed `skills install --help --output-format json` timing out the same way before the broader probe had to be killed, narrowing the gap to subcommand help dispatch rather than the entire skills help surface. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from generic help hangs and from skill-root/provenance issues: the affected surface is skills subcommand help, where claws need bounded machine-readable per-action semantics before listing, installing, or invoking skills. **Required fix shape:** (a) make `skills list --help --output-format json` and sibling skills subcommand help forms return bounded stdout JSON with `kind:"skills"`, `action:"help"`, `subcommand:"list"`, usage, options, output schema, install roots/source notes, examples, and related slash/direct commands; (b) ensure help rendering does not trigger slow skill discovery, filesystem ancestor scans, installs, or runtime invocation; (c) if dynamic skill-root inspection is consulted, return a typed JSON timeout/unavailable error instead of hanging silently; (d) add regression coverage proving skills subcommand help returns within a deterministic budget while top-level skills help remains parseable. **Why this matters:** skills are a plugin-like extension surface. If top-level help is machine-readable but subcommand help hangs, claws can discover that skills exist but cannot safely learn how to list/install/use them before mutating extension state or scanning untrusted roots. Source: gaebal-gajae dogfood follow-up for the 07:30 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
