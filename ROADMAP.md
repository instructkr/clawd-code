...
349. **Top-level `plugins show <missing-plugin> --output-format json` hangs with zero stdout/stderr instead of returning a typed plugin-not-found response** — dogfooded 2026-04-29 for the 21:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `a2a38df9`. After rebuilding and verifying the binary provenance, repeated bounded runs of `timeout 8 ./rust/target/debug/claw plugins show does-not-exist --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `plugins list --output-format json` returned promptly with the known plugin inventory prose payload, proving the plugin top-level surface itself is reachable; the hang is specific to the missing-plugin detail path. This is distinct from #348's prose-only plugin inventory schema: #348 covers list payload shape after success, while this pinpoint covers a detail lookup for an absent plugin never producing any JSON outcome. **Required fix shape:** (a) make `plugins show <plugin> --output-format json` perform a bounded registry/config lookup before any long-running plugin work; (b) return stdout JSON with `kind:"plugin"`, `action:"show"`, `status:"not_found"` or `kind:"error"` plus `code:"plugin_not_found"`, `plugin`, and optional `available_plugins[]`; (c) add internal timeout/lifecycle diagnostics so plugin detail lookup cannot hang silently with no bytes; (d) add regression coverage proving missing-plugin detail lookup returns a typed JSON not-found response within a deterministic budget. **Why this matters:** plugin lifecycle management needs safe inspection before mutation. If a typo or stale plugin name can silently hang, claws cannot distinguish missing plugin, registry scan stall, plugin loader deadlock, or CLI dispatch bug, and any automated enable/disable/update flow loses a safe preflight. Source: gaebal-gajae dogfood follow-up for the 21:30 nudge on rebuilt `./rust/target/debug/claw` `a2a38df9`.
