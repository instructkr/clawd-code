...
347. **Top-level `mcp show <missing-server> --output-format json` is nondeterministic: the same missing-server lookup can return a JSON not-configured object or hang with zero stdout/stderr** — dogfooded 2026-04-29 for the 20:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `ee41b266`. After rebuilding and verifying `./rust/target/debug/claw version --output-format json` reported `git_sha` `ee41b266`, one bounded run of `./rust/target/debug/claw mcp show does-not-exist --output-format json` returned stdout JSON with `{"action":"show","found":false,"kind":"mcp","message":"server `does-not-exist` is not configured","server_name":"does-not-exist","status":"ok"}` and no stderr. Immediately repeated bounded probes of the same command then produced `timeout` exits (`124`) with `stdout=0` and `stderr=0`, while `mcp list --output-format json` still returned promptly. This is distinct from #327's MCP help/source-list mismatch and the earlier invalid #2874 help-hang hypothesis: the pinpoint is nondeterministic missing-server detail lookup, not help discovery. **Required fix shape:** (a) make `mcp show <server> --output-format json` a pure bounded config lookup for missing names, never attempting transport work before confirming the server exists; (b) return one stable schema for missing servers, preferably a typed error or explicit `found:false` object, but not sometimes a silent hang; (c) add internal timeout/lifecycle diagnostics around MCP show detail lookup so any stall produces stdout JSON with `code:"mcp_show_timeout"` / `server`; (d) add regression coverage running repeated missing-server lookups to prove deterministic, bounded behavior. **Why this matters:** MCP diagnostics must be replayable. If the same typo can alternately return a useful not-configured object or hang with no bytes, claws cannot trust retries, cannot classify the failure mode, and may file contradictory evidence depending on timing. Source: gaebal-gajae dogfood follow-up for the 20:30 nudge on rebuilt `./rust/target/debug/claw` `ee41b266`; invalid single-run hang PR #2879 was closed after immediate repro showed the success branch.
